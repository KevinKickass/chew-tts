...
pub fn load_layer_to_gpu(
    ...
    let moe_gate_host = gguf.tensor_data_by_name(&format!("{pfx}.ffn_gate_inp.weight"))
        .map(|(_ti, data)| {
            data.chunks_exact(4)
        ...
        .unwrap_or_default();
    let moe_gate_scale_host = gguf.tensor_data_by_name(&format!("{pfx}.ffn_gate_inp.scale"))
        .map(|(_ti, data)| {
            data.chunks_exact(4)
                .map(|c| f32::from_le_bytes([c[0], c[1], c[2], c[3]]))
                .collect::<Vec<f32>>()
    ...
    let post_norm = if gguf.find_tensor(&format!("{pfx}.post_norm.weight")).is_some() {
        Some(upload_and_dequant(gguf, &format!("{pfx}.post_norm.weight"), alloc, dequant, gpu_idx)?)
    } else {
        None
    };
    let inp_gate = if gguf.find_tensor(&format!("{pfx}.inp_gate.weight")).is_some() {
        Some(upload_quantized(gguf, &format!("{pfx}.inp_gate.weight"), alloc, gpu_idx)?)
    } else {
        None
    };
    let proj = if gguf.find_tensor(&format!("{pfx}.proj.weight")).is_some() {
        Some(upload_quantized(gguf, &format!("{pfx}.proj.weight"), alloc, gpu_idx)?)
    } else {
        None
    };
...
pub fn load_layer_norms(
    ...
    Ok(MoeStreamingNorms {
        ...
        post_norm: if gguf.find_tensor(&format!("{pfx}.post_norm.weight")).is_some() {
            Some(upload_and_dequant(gguf, &format!("{pfx}.post_norm.weight"), alloc, dequant, gpu_idx)?)
        } else {
            None
        },
...
pub fn plan_layer_host_layout(
    ...
    fn tensor_slot(gguf: &GgufFile, name: &str, cursor: &mut usize, base: usize) -> Result<TensorSlot, LoadError> {
        let ti = gguf.find_tensor(name)
    ...
    fn try_tensor_slot(gguf: &GgufFile, name: &str, cursor: &mut usize, base: usize) -> Result<Option<TensorSlot>, LoadError> {
    ...
    let attn_q = tensor_slot(gguf, &format!("{pfx}.attn_q.weight"), &mut cursor, base_offset)?;
    let attn_k = tensor_slot(gguf, &format!("{pfx}.attn_k.weight"), &mut cursor, base_offset)?;
    let attn_v = try_tensor_slot(gguf, &format!("{pfx}.attn_v.weight"), &mut cursor, base_offset)?;
    let attn_output = tensor_slot(gguf, &format!("{pfx}.attn_output.weight"), &mut cursor, base_offset)?;
    let ffn_gate = tensor_slot(gguf, &format!("{pfx}.ffn_gate.weight"), &mut cursor, base_offset)?;
    let ffn_up = tensor_slot(gguf, &format!("{pfx}.ffn_up.weight"), &mut cursor, base_offset)?;
    let ffn_down = tensor_slot(gguf, &format!("{pfx}.ffn_down.weight"), &mut cursor, base_offset)?;
    let moe_gate = tensor_slot(gguf, &format!("{pfx}.ffn_gate_inp.weight"), &mut cursor, base_offset)?;
    let moe_gate_up_exps = tensor_slot(gguf, &format!("{pfx}.ffn_gate_up_exps.weight"), &mut cursor, base_offset)?;
    let moe_down_exps = tensor_slot(gguf, &format!("{pfx}.ffn_down_exps.weight"), &mut cursor, base_offset)?;
    let inp_gate = try_tensor_slot(gguf, &format!("{pfx}.inp_gate.weight"), &mut cursor, base_offset)?;
    let proj = try_tensor_slot(gguf, &format!("{pfx}.proj.weight"), &mut cursor, base_offset)?;
...
pub fn fill_layer_host_data(
    ...
    fn copy_tensor(gguf: &GgufFile, name: &str, dst: &mut [u8], slot: &TensorSlot) -> Result<(), LoadError> {
        let (_ti, data) = gguf.tensor_data_by_name(name)
    ...
    copy_tensor(gguf, &format!("{pfx}.attn_output.weight"), dst, &layout.attn_output)?;
    copy_tensor(gguf, &format!("{pfx}.ffn_gate.weight"), dst, &layout.ffn_gate)?;
    copy_tensor(gguf, &format!("{pfx}.ffn_up.weight"), dst, &layout.ffn_up)?;
    copy_tensor(gguf, &format!("{pfx}.ffn_down.weight"), dst, &layout.ffn_down)?;
    copy_tensor(gguf, &format!("{pfx}.ffn_gate_inp.weight"), dst, &layout.moe_gate)?;
    copy_tensor(gguf, &format!("{pfx}.ffn_gate_up_exps.weight"), dst, &layout.moe_gate_up_exps)?;
    copy_tensor(gguf, &format!("{pfx}.ffn_down_exps.weight"), dst, &layout.moe_down_exps)?;
...
pub fn upload_to_shell(
    ...
    fn upload_slot(host_slice: &[u8], base: usize, slot: &TensorSlot, qw: &mut QuantWeight, stream: &Arc<CudaStream>) -> Result<(), String> {
        let src = &host_slice[base + slot.off..base + slot.off + slot.size];
        let mut dst = qw.data.slice_mut(0..slot.size);
        stream.memcpy_htod(src, &mut dst).map_err(|e| e.to_string())?;
    ...
    upload_slot(host_slice, base, &layout.attn_output, &mut shell.attn_output, stream)?;
...
fn enqueue_shell_prefetch(
    ...
    if let Some(done) = wait_on {
        moe_weights.dma_stream.wait(done)
    ...
    let (htod_us, dtod_us) = upload_to_shell(&moe_weights.host_layer_data, layout, shell, &moe_weights.dma_stream)
        .map_err(KernelError::Launch)?;
    let t_norms = std::time::Instant::now();
    copy_norms_to_shell(norms, shell, &moe_weights.dma_stream)
        .map_err(KernelError::Launch)?;
    let norms_us = t_norms.elapsed().as_micros();
    let t_event = std::time::Instant::now();
    let ev = moe_weights.dma_stream.record_event(None)
        .map_err(|e| KernelError::Launch(e.to_string()))?;
    Ok((ev, htod_us, dtod_us, norms_us, t_event.elapsed().as_micros()))
}
...
impl MoeModelWeights {
    ...
    pub fn load(
        ...
        let per_layer_token_embd = if config.embd_per_layer.is_some() && gguf.find_tensor("per_layer_token_embd.weight").is_some() {
            Some(upload_quantized(gguf, "per_layer_token_embd.weight", alloc, gpu_idx)?)
        } else {
            None
        };
        let per_layer_model_proj = if config.embd_per_layer.is_some() && gguf.find_tensor("per_layer_model_proj.weight").is_some() {
            Some(upload_quantized(gguf, "per_layer_model_proj.weight", alloc, gpu_idx)?)
        } else {
            None
        };
        let per_layer_proj_norm = if config.embd_per_layer.is_some() && gguf.find_tensor("per_layer_proj_norm.weight").is_some() {
        ...
        info!(host_mb = total_host_bytes / (1024 * 1024), "MoE streamed layer layout planned");

        let mut host_layer_data = unsafe {
            stream.context().alloc_pinned::<u8>(total_host_bytes)
        }.map_err(|e| LoadError::Vram(chew_vram::VramError::Alloc(e.to_string())))?;
        let host_slice = host_layer_data
            .as_mut_slice()
            .map_err(|e| LoadError::Vram(chew_vram::VramError::Alloc(e.to_string())))?;
        for (host_idx, layer_idx) in (n_resident..n_layers).enumerate() {
            info!(layer = layer_idx, "loading MoE streamed layer to pinned host RAM");
            let layout = &host_layer_offsets[host_idx];
            fill_layer_host_data(gguf, layer_idx, &mut host_slice[layout.offset..layout.offset + layout.size], layout)?;
        }
        info!(host_mb = total_host_bytes / (1024 * 1024), "MoE streamed layer data loaded into pinned host RAM");
        ...
        let dma_stream = stream.context().new_stream()
            .map_err(|e| LoadError::Vram(chew_vram::VramError::Alloc(e.to_string())))?;
        let expert_dma_stream = stream.context().new_stream()
            .map_err(|e| LoadError::Vram(chew_vram::VramError::Alloc(e.to_string())))?;

        // Expert DMA scratch: one slot per routed expert, sized to actual quantized slices.
        let expert_gate_up_bytes = host_layer_offsets.iter()
            .map(|l| l.moe_gate_up_exps.size / config.n_experts as usize)
            .max()
            .unwrap_or(64)
            .max(64);
        let expert_down_bytes = host_layer_offsets.iter()
        ...
        let mk_expert_qw = |size: usize| -> Result<QuantWeight, LoadError> {
            let data = stream.alloc_zeros::<u8>(size)
                .map_err(|e| LoadError::Vram(chew_vram::VramError::Alloc(e.to_string())))?;
            Ok(QuantWeight { data, quant_type: GgmlType::F16, n_elements: 0 })
        };
        let n_expert_slots = (config.n_experts_per_tok as usize * 4).max(config.n_experts_per_tok as usize);
        ...
        let d = config.dim as usize;
        let expert_weight_buf = stream.alloc_zeros::<f32>(config.n_experts_per_tok as usize)
            .map_err(|e| LoadError::Vram(chew_vram::VramError::Alloc(e.to_string())))?;
        let expert_gate_up_batch_out = stream.alloc_zeros::<half::f16>(config.n_experts_per_tok as usize * expert_ff * 2)
            .map_err(|e| LoadError::Vram(chew_vram::VramError::Alloc(e.to_string())))?;
        ...
        info!(n_resident = model.n_resident, n_streamed = model.host_layer_offsets.len(), "MoE streaming weights loaded");
...
pub fn forward_moe_streaming(
    ...
    if n_layers > moe_weights.n_resident {
        ...
        if profile { t_prefetch_submit += t0.elapsed().as_micros(); }
    }
    ...
    for layer_idx in 0..n_layers {
        ...
        let layer: &MoeLayerWeights = if layer_idx < moe_weights.n_resident {
            ...
            if let Some(ready) = &shell_ready[slot] {
                timed!(t_stream, stream.wait(ready)).map_err(|e| KernelError::Launch(e.to_string()))?;
            } else {
                let t0 = std::time::Instant::now();
                let (ev, htod, dtod, norms, event_us) = enqueue_shell_prefetch(moe_weights, host_idx, slot, shell_done[slot].as_ref())?;
            ...
            let shell_ref = if slot == 0 { &moe_weights.shell_a } else { &moe_weights.shell_b };
            shell_ref
        };

        if debug_decode {
            info!(layer_idx, hd, kv_heads, is_swa, is_resident = layer_idx < moe_weights.n_resident, q_dim = config.n_heads * hd, kv_dim = kv_heads * hd, "MoE layer start");
        }
        ...
        if seq_len == 1 && !is_resident {
            timed!(t_gemm, kernels.gemv.quantize_input(&scratch.norm_out, config.dim))?;
        }
        ...
        let _g0 = t_gemm;
        timed!(t_gemm, gemm(kernels, &scratch.norm_out, &layer.attn_q, &mut scratch.q,
            seq_len, q_dim, config.dim))?;
        if profile { t_gemm_qkv += t_gemm - _g0; }
        sync_check!("Q_proj");
        if layer.has_attn_v && seq_len == 1 && is_resident {
            ...
            if layer.attn_k.quant_type == attn_v.quant_type {
                ...
                let used = timed!(t_gemm, kernels.gemv.gemv_dual(
                    &layer.attn_k.data, &attn_v.data,
                    &mut scratch.k, &mut scratch.v,
                    kv_dim, config.dim, layer.attn_k.quant_type,
                ))?;
                if !used {
                    timed!(t_gemm, gemm(kernels, &scratch.norm_out, &layer.attn_k, &mut scratch.k,
                        seq_len, kv_dim, config.dim))?;
                    timed!(t_gemm, gemm(kernels, &scratch.norm_out, attn_v, &mut scratch.v,
                        seq_len, kv_dim, config.dim))?;
                }
                if profile { t_gemm_qkv += t_gemm - _g0; }
            } else {
        ...
        // V norm (no weight)
        {
            ...
            unsafe {
                timed!(t_norm, kernels.ops.rms_norm_no_weight(
                    &*src_ptr, &mut *dst_ptr,
                    seq_len * kv_heads, hd, config.rms_norm_eps,
                ))?;
            }
        ...
        if has_kv {
            let kv_elems = seq_len * kv_dim;
            {
                ...
                timed!(t_kv, kernels.ops.copy_f16(&scratch.k, &mut k_cache, kv_elems))?;
            }
            {
                let mut v_cache = kv_cache.v_mut(layer_idx, seq_len);
                timed!(t_kv, kernels.ops.copy_f16(&scratch.v, &mut v_cache, kv_elems))?;
            }
        }
        ...
        // ── 6. Multi-Head Attention ──
        {
            ...
            if !is_swa {
                timed!(t_full_mha, kernels.ops.mha_naive(
                    &scratch.q,
                    &k_full,
                    &v_full,
                ...
                timed!(t_swa_mha, kernels.ops.mha_fused_scaled(
        ...
        // ── 7. Output projection ──
        if seq_len == 1 && is_resident {
            timed!(t_gemm, kernels.gemv.quantize_input(&scratch.attn_mha_out, q_dim))?;
        }
        let _g0 = t_gemm;
        timed!(t_gemm, gemm(kernels, &scratch.attn_mha_out, &layer.attn_output, &mut scratch.attn_out,
            seq_len, config.dim, q_dim))?;
        if profile { t_gemm_attn_out += t_gemm - _g0; }
        ...
        timed!(t_add, kernels.ops.post_norm_add(
        ...
        let _g0 = t_gemm;
        timed!(t_gemm, gemm(kernels, &scratch.norm_out, &layer.ffn_gate, &mut scratch.ffn_gate_out,
            seq_len, config.ff_dim, config.dim))?;
        timed!(t_gemm, gemm(kernels, &scratch.norm_out, &layer.ffn_up, &mut scratch.ffn_up_out,
            seq_len, config.ff_dim, config.dim))?;
        if profile { t_gemm_ffn_upgate += t_gemm - _g0; }

        timed!(t_norm, kernels.ops.gelu(
        ...
        let _g0 = t_gemm;
        timed!(t_gemm, gemm(kernels, &scratch.ffn_silu_out, &layer.ffn_down, &mut scratch.ffn_out,
        ...
        timed!(t_add, stream.memcpy_dtod(&scratch.attn_out, &mut scratch.ffn_out))
            .map_err(|e| KernelError::Launch(e.to_string()))?;
        ...
        //            logits = tmp @ gate_inp
        {
            ...
            for tok in 0..seq_len as usize {
                ...
                for (expert_pos, &(expert_id, weight)) in selected.iter().enumerate() {
                    ...
                    let resident_slices = if streamed_layout.is_some() {
                        if let Some(ready) = expert_slots[slot_idx].ready.as_ref() {
                            ...
                            if profile {
                                ...
                                if selected_cache_hits.get(expert_pos).copied().unwrap_or(false) {
                    ...
                    if streamed_layout.is_some() {
                        ...
                        timed!(t_expert, kernels.gemm.matmul_dequant(
                            &scratch.norm_out, &gate_up_qw.data,
                            gate_up_qw.quant_type, gate_up_qw.n_elements,
                    ...
                    if profile { t_expert_down += t_expert - _e0; }

                    if first_expert {
                        let _e0 = t_expert;
                        timed!(t_expert, kernels.ops.scale_f16(&scratch.attn_out, &mut scratch.attn_mha_out,
                            config.dim, w_norm))?;
                        if profile { t_expert_accum += t_expert - _e0; }
                        first_expert = false;
                    } else {
                        let src_ptr = &scratch.attn_out as *const CudaSlice<half::f16>;
                        let dst_ptr = &mut scratch.attn_out as *mut CudaSlice<half::f16>;
                        let _e0 = t_expert;
                        unsafe { timed!(t_expert, kernels.ops.scale_f16(&*src_ptr, &mut *dst_ptr, config.dim, w_norm))?; }
                        ...
                        unsafe { timed!(t_expert, kernels.ops.add_f16(&*a, &*b, &mut *c, config.dim))?; }
                        if profile { t_expert_accum += t_expert - _e0; }
                    }
                }
                ...
                if seq_len > 1 && tok > 0 {
                    ...
                    timed!(t_add, stream.memcpy_dtod(&src2, &mut dst))
                        .map_err(|e| KernelError::Launch(e.to_string()))?;
                }
            }
        ...
        if let (Some(inp_gate), Some(proj), Some(post_norm), Some(pe_data), Some(epl)) = (
            ...
            let _g0 = t_gemm;
            timed!(t_gemm, gemm(kernels, &scratch.norm_out, inp_gate, pe_gate, seq_len, epl, config.dim))?;
            if profile { t_gemm_pe += t_gemm - _g0; }

            {
                let src_ptr = pe_gate as *const CudaSlice<half::f16>;
                let dst_ptr = pe_gate as *mut CudaSlice<half::f16>;
                unsafe {
                    timed!(t_norm, kernels.ops.gelu_act(&*src_ptr, &mut *dst_ptr, seq_len * epl))?;
                }
            }

            {
                ...
                unsafe {
                    timed!(t_add, kernels.ops.pe_strided_mul(
            ...
            let _g0 = t_gemm;
            timed!(t_gemm, gemm(kernels, pe_gate, proj, pe_proj, seq_len, config.dim, epl))?;
            if profile { t_gemm_pe += t_gemm - _g0; }
        ...
        if let Some(scale) = layer.layer_output_scale {
            timed!(t_add, kernels.ops.scale_f32_inplace(hidden, seq_len * config.dim, scale))?;
        }
        ...
        if layer_idx >= moe_weights.n_resident {
            let host_idx = layer_idx - moe_weights.n_resident;
            let slot = host_idx % 2;
            shell_done[slot] = Some(stream.record_event(None).map_err(|e| KernelError::Launch(e.to_string()))?);
            let next_streamed = host_idx + 2;
            if next_streamed < moe_weights.host_layer_offsets.len() {
                let t0 = std::time::Instant::now();
                let (ev, htod, dtod, norms, event_us) = enqueue_shell_prefetch(moe_weights, next_streamed, slot, shell_done[slot].as_ref())?;
                shell_ready[slot] = Some(ev);
                if profile {
                    t_prefetch_htod += htod;
                    t_prefetch_dtod += dtod;
    ...
    if debug_decode {
        info!(pos_before_advance = kv_cache.pos(), seq_len, "kv_cache before advance");
    }
    ...
    let _g0 = t_gemm;
    timed!(t_gemm, gemm_q(kernels, &scratch.norm_out, &moe_weights.output, &mut scratch.logits,
        seq_len, config.vocab_size, config.dim))?;
    if profile { t_gemm_logits += t_gemm - _g0; }
    ...
    if profile {
        let total = t_stream + t_norm + t_gemm + t_rope + t_kv + t_swa_mha + t_full_mha + t_router + t_expert + t_add;
        info!(
            stream_us = t_stream,
            norm_us = t_norm,
            gemm_us = t_gemm,
...
fn enqueue_streamed_expert_prefetch(
    ...
    if let Some(done) = wait_on {
        dma_stream.wait(done)
            .map_err(|e| KernelError::Launch(e.to_string()))?;
    }

    let host = host_layer_data.as_slice()
        .map_err(|e| KernelError::Launch(e.to_string()))?;
    let slot = &mut expert_slots[slot_idx];
    ...
    dma_stream.memcpy_htod(
        gu_src,
        &mut slot.gate_up.data.slice_mut(0..gu.byte_size),
    ).map_err(|e| KernelError::Launch(e.to_string()))?;
    ...
    dma_stream.memcpy_htod(
        dn_src,
        &mut slot.down.data.slice_mut(0..dn.byte_size),
    ).map_err(|e| KernelError::Launch(e.to_string()))?;
    slot.down.quant_type = dn.quant_type;
    slot.down.n_elements = dn.n_elements;

    dma_stream.record_event(None)
        .map_err(|e| KernelError::Launch(e.to_string()))
}
