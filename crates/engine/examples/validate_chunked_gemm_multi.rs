...
fn main() {
    ...
    gemm_full
        .matmul_f16(&a_gpu, &b_gpu, &mut c_ref_gpu, m, n, k)
        .unwrap();
    ...
    stream
        .memcpy_dtoh(&c_chunked_gpu, &mut c_chunked_f16)
        .unwrap();
    let c_chunked: Vec<f32> = c_chunked_f16.iter().map(|v| v.to_f32()).collect();

    let max_err_chunked = c_cpu
        .iter()
        .zip(&c_chunked)
        .map(|(c, g)| (c - g).abs())
        .fold(0.0f32, f32::max);
    println!(
        "  Chunked GEMM vs CPU max_err: {:.6} {}",
        max_err_chunked,
        if max_err_chunked < 0.05 {
            "PASS"
        } else {
            "FAIL"
        }
