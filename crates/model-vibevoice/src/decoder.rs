...
fn conv1d_causal_gemm(
    ...
    kernels
        .ops
        .unfold_causal_f16(input, &mut unfolded, in_channels, frames, kernel_size, 1)?;
...
fn conv_transpose1d_causal_gemm(
    ...
    kernels: &mut GpuKernels,
        ...
        "VibeVoice transposed convolution phase count disagrees"
    );
    let stream = Arc::clone(kernels.ops.stream());
...
fn load_conv_transpose(
    ...
    let input_channels = weight.shape[0];
    let output_channels = weight.shape[1];
    let kernel = weight.shape[2];
    let phase_weights = (0..stride)
        .map(|phase| {
            let mut packed = vec![f16::ZERO; output_channels * input_channels * 2];
            for output in 0..output_channels {
                for input in 0..input_channels {
                    let source = (input * output_channels + output) * kernel;
                    let destination = (output * input_channels + input) * 2;
                    packed[destination] = weight.values[source + phase + stride];
                    packed[destination + 1] = weight.values[source + phase];
                }
            }
