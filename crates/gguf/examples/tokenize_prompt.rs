...
fn main() {
    let model = std::env::args().nth(1).expect("usage: tokenize_prompt <model.gguf> <prompt-file>");
    let prompt_file = std::env::args().nth(2).expect("usage: tokenize_prompt <model.gguf> <prompt-file>");
